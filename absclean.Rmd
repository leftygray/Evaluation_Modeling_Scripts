...
dataYear <- 2023 # Annual data from 2014
...
erpData <- read_excel(file.path(absFolder, 
  paste0("310104-ABS_ERP_State_Overall-", dataYear, ".xlsx")),
  sheet = 2, skip = 9) |>
  # Delete unnecessary columns (keeping the last nine) 
  dplyr::select(c(1, 20:ncol(erpData))) |>
  mutate(`Series ID` = year(`Series ID`)) # Extract year
...
# long-term resident, long-term visitor, total) and extract year
departData <- departData |>
  dplyr::select(c(1:5)) |>
  mutate(`Series ID` = year(`Series ID`)) 
...
departData <- departData |>
  ...
  mutate(total_lt = resident + visitor) |> # calculate total long term departures
  mutate(
