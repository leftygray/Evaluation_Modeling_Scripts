...
# For 2023 rename variables  
nndssdata <- nndssdata |>
  rename(REMOTE = remoteness, diagnosisdate = DIAG_DTE)
...
ctOverall <- chlamdata %>%
  filter(year %in% years, !is.na(remote)) %>%
  group_by(year) %>%
  summarise(n = n())

ctUrban <- chlamdata %>%
  filter(year %in% years, remote == 1) %>%
  group_by(year) %>%
  summarise(n = n())

ctRegional <- chlamdata %>%
  filter(year %in% years, remote == 2) %>%
  group_by(year) %>%
  summarise(n = n())
