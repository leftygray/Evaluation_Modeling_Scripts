...
if (dataYear <= 2013) {
  ...
  hivdata <- read.csv(rawdata, colClasses = c("character", "character", 
   ...
   hivdata <- read.csv(rawdata, colClasses = c("character", "character", 
    "Date",  "numeric", "character", "Date", "numeric", "Date", "numeric", 
    "character", "numeric", "Date", "Date", "Date", "Date", "numeric",
    "numeric", "numeric", "Date", "numeric", "numeric", "numeric", "numeric"))
}
...
# Add a sero-conversion indicator
# cli_status_on_spec_dt == 2 or dateneg < 365 before diagnosis. 
# Important for distinguishing late diagnosis by CD4 count. 
hivdata$seroconversion <- "no"

hivdata <- hivdata |>
  mutate(cli_status_on_spec_dt = ifelse(is.na(cli_status_on_spec_dt), 0,
    cli_status_on_spec_dt)) |>
  mutate(seroconversion = ifelse(cli_status_on_spec_dt == 2, "yes",
    seroconversion)) |>
  mutate(days_since_neg = datediagnosis - dateneg) |>
  # Convert NAs to a large number
  mutate(days_since_neg = ifelse(is.na(days_since_neg), 10000,
    days_since_neg)) |>
  mutate(seroconversion = ifelse(seroconversion != 2 & days_since_neg < 365,
    "yes", seroconversion)) |>
  select(-days_since_neg)
