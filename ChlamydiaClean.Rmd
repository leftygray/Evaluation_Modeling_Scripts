...
cascadeDataFolder <- file.path("C:", "Users", "rgray", "UNSW",
  "Australian Care Cascades - Documents", "Chlamydia", "data")
...
```{r notifications}
# Raw notifications file
# Only works for 2014 and from 2019 at this stage
if (dataYear == 2014) {
  
  rawdata <- file.path(dataPath, paste0("ChlamydiaNotifications-",
    toString(dataYear), ".csv"))
  
  # Load the raw notifications file and clean it up
  ...
  # Extract the chlamydia notifications and key variables. 
  # Clean up dataframe
  chlamdata <- tibble(nndssdata) %>%
    filter(diseasecode == 7) %>%
    select(age = AGE, sex = SEX, year = YEAR) %>%
    mutate(sex = tolower(sex)) %>%
    filter(year %in% (dataYear-10+1):dataYear)
  # 
  ...
  # Check for missing values and replace with NAs. 
  # Replace '-1's with NAs for age keep missing for sex
  chlamdata[chlamdata$age == -1,]$age <- NA
  ...
  #Bind into a wide data frame
  chlamNotifications <- bind_cols(temp1, temp15, temp20, temp25, temp30,
    temp35, tempAll, tempMiss)
  
  # Re-arrange to match model input
  chlamNotifications <- chlamNotifications %>% 
    select(year = "year...1", starts_with("male"), starts_with("female"),
      everything()) %>%
    select(-contains("..."))
  ...
  finalNotifications <- knownNotifications
  finalNotifications[, 2:ncol(finalNotifications)] <- 
    knownNotifications[, 2:ncol(finalNotifications)] + extraNotifications
  ...
  # Save cleaned chlamdata
  write.csv(finalNotifications, file = file.path(cleanDataFolder,
    paste0("chlamnotifications_clean-", dataYear, ".csv")), 
    row.names = FALSE)
  
  write.csv(finalNotifications, file = file.path(cascadeDataFolder,
    dataYear, paste0("chlamnotifications_clean-", dataYear, ".csv")), 
    row.names = FALSE)
  
  
} else {
  stop("Raw notifications file not available") 
}
```

## Chlamydia tests

Mostly deprecated as the data is now provided in the right format. 
...
```{r re-testing ACCESS - Before 2017}
if (dataYear < 2017) {
  ...
  # Manually relabel columns by all, male and female data
...
if (dataYear == 2017) {
  ...
  # Load raw re-testing data
  chlamRetest <- read_excel(rawDataTreatment, sheet = 5, skip = 4,
    col_names = FALSE)
  
  # Clean up data ----------------------------------------------------------
  # Lots of hardcoding
  chlamRetest <- chlamRetest[63:89, 1:24]
  
  # Separate by sex and context
  chlamRetestmshc <- chlamRetest %>% 
    select(1:4) %>%
    rename(year = X__1, region = X__2, retested = X__3, diagnoses = X__4) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "male", context = "shc") %>%
    select(region, context, sex, year, diagnoses, retested) 
  
  chlamRetestfshc <- chlamRetest %>% 
    select(7:10) %>%
    rename(year = X__7, region = X__8, retested = X__9, diagnoses = X__10) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "female", context = "shc") %>%
    select(region, context, sex, year, diagnoses, retested)
  
  chlamRetestmgp <- chlamRetest %>% 
    select(13:16) %>%
    rename(year = X__13, region = X__14, retested = X__15, diagnoses = X__16) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "male", context = "gp") %>%
    select(region, context, sex, year, diagnoses, retested)
  
  chlamRetestfgp <- chlamRetest %>% 
    select(19:22) %>%
    rename(year = X__19, region = X__20, retested = X__21, diagnoses = X__22) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "female", context = "gp") %>%
    select(region, context, sex, year, diagnoses, retested)
  
  # Merge and filter out 2017 data
  chlamRetest <- bind_rows(chlamRetestmshc, chlamRetestfshc, 
    chlamRetestmgp, chlamRetestfgp) %>%
    filter(year == 2017) 
  
  # Add zeros for remote GPs
  chlamRetest <- chlamRetest %>%
    bind_rows(data_frame(region = "remote", context = "gp", 
      sex = c("all", "male", "female"), year = 2017, diagnoses = 0, 
      retested = 0))
  
  # Do overall calculations
  chlamRetest <- chlamRetest %>%
    group_by(region, context, year) %>%
    summarise(diagnoses = sum(diagnoses), retested = sum(retested)) %>%
    ungroup() %>%
    mutate(sex = "all") %>%
    select(region, context, sex, year, diagnoses, retested) %>%
    bind_rows(chlamRetest) %>%
    arrange(region, context, sex)
  
  if (mergeRemote) {
    # Add another set of rows for regional+remote data
    chlamRetest <- chlamRetest %>%
      filter(region %in% c("regional", "remote")) %>%
      group_by(context, year, sex) %>%
      summarise(diagnoses = sum(diagnoses), retested = sum(retested)) %>%
      ungroup() %>%
      mutate(region = "regional+remote") %>%
      select(region, context, sex, year, diagnoses, retested) %>%
      bind_rows(chlamRetest, .)
  }
  ...
  # Replace 0/0 calculation with NA
  chlamRetest[!indices, ]$prop_retested <- NA
  
  # Save SHC data for range
  chlamRetestRange <- chlamRetest %>% 
    filter(context == "shc", region != "regional+remote") %>%
    mutate(period = "42-180") %>%
    select(region, context, sex, year, diagnoses, period, retested, 
      prop_retested, prop_lower, prop_upper)
...
```{r re-testing ACCESS - 2018-2019}
if (dataYear %in% 2018:2019) {
  # ACCESS re-testing data
  dataStartYear <- 2009
  mergeRemote <-TRUE # add remote and regional data
  
  # Raw testing file
  rawDataTreatment <- file.path(dataPath, paste0("access_cascade_data_CT-", 
    toString(dataYear), ".xlsx"))
  
  # Load raw re-testing data
  chlamRetest <- read_excel(rawDataTreatment, sheet = 1, skip = 2,
    col_names = FALSE) 
  
  # Clean up data ---------------------------------------------------------
  newNames <- c("context", "region", "sex", "indicator",
    as.character(dataStartYear:dataYear))
  
  chlamRetest <- chlamRetest %>%
    rename_at(1:length(newNames), function(x) newNames) %>%
    fill(context, region, sex) %>%
    gather(5:length(newNames), key = "year", value = "data") %>%
    spread(indicator, data) %>%
    rename_at(5:7, function(x) c("diagnoses", "retested", 
      "prop_retested")) %>%
    # filter(year == dataYear) %>%
    mutate(context = case_when(context == "General practices" ~ "gp", 
      context == "Sexual health clinics" ~ "shc")) %>%
    mutate(sex = tolower(sex)) %>%
    # filter(year == dataYear) %>%
    replace(is.na(.), 0)
  ...
  chlamRetest <- chlamRetest %>%
    mutate(prop_retested = retested / diagnoses) #%>%
  # mutate(propLower = ifelse(diagnoses != 0, 
  # LowerProp(retested, diagnoses), 0))
  
  # Need to remove NAs to calculate upper and lower 95% confidence
  # intervals using prop.test
  ...
  chlamRetest <- data.frame(chlamRetest) # Needed to convert to dataframe
  # from tibble for next lines to 
  # work
  chlamRetest[indices, ]$prop_lower <- propLower
  chlamRetest[indices, ]$prop_upper <- propUpper
  ...
  # Arrange into the required format
  chlamRetest <- chlamRetest %>%
    arrange(match(region, c("urban", "regional", "remote"))) %>%
...
```{r re-testing ACCESS - 2021 ASR}
if (dataYear >= 2020) {
  # ACCESS re-testing data
  dataStartYear <- 2010
  mergeRemote <-TRUE # add remote and regional data
  numyears <- 5 # Number of years for cascade
  ...
  chlamRetest <- chlamRetest %>%
    filter(year %in% c((dataYear-numyears+1):dataYear)) %>%
    arrange(match(region, c("urban", "regional", "remote"))) %>%
    arrange(match(context, c("shc", "gp"))) %>%
    arrange(match(sex, c("male", "female", "all"))) %>%
    arrange(year)
...
if (dataYear == 2017) {
  ...
  # Load raw re-testing data
  chlamRetestSHC <- read_excel(rawDataTreatment, sheet = 5, skip = 4,
    col_names = FALSE)
  
  # Clean up data ----------------------------------------------------------
  # Lots of hardcoding
  chlamRetestSHC120 <- chlamRetestSHC[1:27, 1:10]
  chlamRetestSHC7 <- chlamRetestSHC[32:58, 1:10]
  
  # Separate by sex and context
  chlamRetest120m <- chlamRetestSHC120 %>% 
    select(1:4) %>%
    rename(year = X__1, region = X__2, retested = X__3, diagnoses = X__4) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "male", context = "shc", period = "42-120") %>%
    select(region, context, sex, year, diagnoses, period, retested)
  
  chlamRetest120f <- chlamRetestSHC120 %>% 
    select(6:9) %>%
    rename(year = X__6, region = X__7, retested = X__8, diagnoses = X__9) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "female", context = "shc", period = "42-120") %>%
    select(region, context, sex, year, diagnoses, period, retested)
  
  chlamRetest7m <- chlamRetestSHC7 %>% 
    select(1:4) %>%
    rename(year = X__1, region = X__2, retested = X__3, diagnoses = X__4) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "male", context = "shc", period = "7-180") %>%
    select(region, context, sex, year, diagnoses, period, retested)
  
  chlamRetest7f <- chlamRetestSHC7 %>% 
    select(6:9) %>%
    rename(year = X__6, region = X__7, retested = X__8, diagnoses = X__9) %>%
    mutate(year = as.integer(year), retested = as.numeric(retested), 
      diagnoses = as.numeric(diagnoses)) %>%
    mutate(sex = "female", context = "shc", period = "7-180") %>%
    select(region, context, sex, year, diagnoses, period, retested)
  
  # Merge and filter out 2017 data
  chlamRetestSHC <- bind_rows(chlamRetest120m, chlamRetest120f, 
    chlamRetest7m, chlamRetest7f) %>%
    filter(year == 2017) 
  
  # Do overall calculations
  retestVariation <- chlamRetestSHC %>%
    group_by(region, context, year, period) %>%
    summarise(diagnoses = sum(diagnoses), retested = sum(retested)) %>%
    ungroup() %>%
    mutate(sex = "all") %>%
    select(region, context, sex, year, diagnoses, period, retested) %>%
    bind_rows(chlamRetestSHC) %>%
    arrange(period, region, sex)
  ...
  # Add base data and write to file ---------------------------------------
  retestVariation <- retestVariation %>% 
    bind_rows(chlamRetestRange) %>%
    arrange(period, region, sex)
